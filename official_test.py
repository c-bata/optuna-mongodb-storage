...
class StorageSupplier:
    ...
    def __enter__(
        ...
        if self.storage_specifier == "mongodb":
...
def test_create_new_study(storage_mode: str) -> None:
    ...
    with StorageSupplier(storage_mode) as storage:
        ...
        assert all(
            s.study_name.startswith(DEFAULT_STUDY_NAME_PREFIX) for s in summaries
        )
...
def test_delete_study_after_create_multiple_studies(storage_mode: str) -> None:
    ...
    with StorageSupplier(storage_mode) as storage:
        ...
        studies = {
            s._study_id: s
            for s in storage.get_all_study_summaries(include_best_trial=True)
        }
        ...
        for target, opposite in [
            ...
            with pytest.raises(KeyError):
                storage.set_study_directions(
                    non_existent_study_id, (StudyDirection.NOT_SET,)
                )
...
def test_create_new_trial(storage_mode: str) -> None:
    ...
    with StorageSupplier(storage_mode) as storage:
        ...
        for i in range(n_trial_in_study):
            ...
            trials = storage.get_all_trials(study_id)
            _check_trials(
                trials, i, trial_id, time_before_creation, time_after_creation
            )
...
def test_create_new_trial_with_template_trial(storage_mode: str) -> None:
    ...
    def _check_trials(trials: List[FrozenTrial], idx: int, trial_id: int) -> None:
        ...
        assert all(
            t.intermediate_values == template_trial.intermediate_values for t in trials
        )
        ...
        assert all(
            t.datetime_complete == template_trial.datetime_complete for t in trials
        )
...
def test_set_trial_state(storage_mode: str) -> None:
    ...
    with StorageSupplier(storage_mode) as storage:
        ...
        if isinstance(storage, (InMemoryStorage, _CachedStorage)):
            pytest.skip(
                "InMemoryStorage and _CachedStorage does not have set_trial_state()"
            )
...
def test_set_trial_param(storage_mode: str) -> None:
    ...
    with StorageSupplier(storage_mode) as storage:
        ...
        distribution_y_1 = CategoricalDistribution(
            choices=("Shibuya", "Ebisu", "Meguro")
        )
...
def test_set_trial_values(storage_mode: str) -> None:
    ...
    with StorageSupplier(storage_mode) as storage:
        ...
        if isinstance(storage, (InMemoryStorage, _CachedStorage)):
            pytest.skip(
                "InMemoryStorage and _CachedStorage does not have set_trial_values()"
            )
            return  # needed for mypy
        # Setup test across multiple studies and trials.
        study_id = storage.create_new_study()
...
def test_get_all_trials_state_option(storage_mode: str) -> None:
    ...
    with StorageSupplier(storage_mode) as storage:
        ...
        trials = storage.get_all_trials(
            study_id, states=(TrialState.COMPLETE, TrialState.PRUNED)
        )
        assert len(trials) == 3
...
def test_get_n_trials(storage_mode: str) -> None:
    ...
    with StorageSupplier(storage_mode) as storage:
        study_id_to_summaries, _ = _setup_studies(
            storage, n_study=2, n_trial=7, seed=50
        )
            ...
            if trial.state == TrialState.COMPLETE and trial.value is not None:
                if best_trial is None:
                    ...
                    if (
                        ...
                        and trial.value < best_trial.value
                    ):
                        best_trial = trial
                    elif (
                        direction == StudyDirection.MAXIMIZE
                        and best_trial.value < trial.value
                    ):
                        best_trial = trial
...
def test_get_best_trial_for_multi_objective_optimization(storage_mode: str) -> None:
    ...
    with StorageSupplier(storage_mode) as storage:
        ...
        storage.set_study_directions(
            study_id, (StudyDirection.MAXIMIZE, StudyDirection.MINIMIZE)
        )
        generator = random.Random(51)
        for i in range(3):
            template_trial = _generate_trial(generator)
...
def test_pickle_storage(storage_mode: str) -> None:
    ...
    with StorageSupplier(storage_mode) as storage:
        ...
        restored_storage_system_attrs = restored_storage.get_study_system_attrs(
            study_id
        )
        assert (
            storage_system_attrs == restored_storage_system_attrs == {"key": "pickle"}
        )
        ...
        if isinstance(storage, RDBStorage):
            ...
            assert (
                storage.skip_compatibility_check
                == restored_storage.skip_compatibility_check
            )
